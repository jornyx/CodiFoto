...
export const takeSnap = async (config) => {
  ...
  if (config.fondoTransparente || config.destino === 'Ventana') {
    setVar('container-background-color', 'transparent');
  }

  const target = config.destino === 'Contenedor' ? snippetContainerNode : windowNode;
  ...
  if (config.accionDelBoton === 'Capturar') {
    ...
    vscode.postMessage({ type: config.accionDelBoton, data });
  }

  windowNode.style.resize = 'horizontal';
  setVar('container-background-color', config.colorDeFondo);
};